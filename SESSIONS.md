...
**Voie active (07/07/2026, nuit)** : **REFONTE UX/UI R1→R10 CODE-COMPLÈTE sur la PR #37 (draft)** — session 30 : étape 1 (rôle UX/UI demandé par le PO : audit des 8 écrans, interview PO en 3 lots → **13 arbitrages UX1–UX13** dont conformité DSFR stricte, maquette navigable `docs/maquettes/refonte-ux-ui.html`, spec + registre d'hypothèses H1–H15 dans `docs/refonte-ux-ui.md`), puis étape 2 (développement, un commit par story, hypothèses structurantes validées par le PO en chemin — H7 fragments htmx, H9 suppression libre) : **R1** socle DSFR (en-tête officiel, nav état actif, notice D15 fine, fil d'Ariane), **R2** écran session refondu (barre + stepper A5, chat chronologique à défilement interne, composeur unifié, rail 3 panneaux Stories/Hypothèses/Sources — R5 absorbée), **R3** fragments htmx ciblés (bulles ajoutées au fil + OOB stepper/conso/panneaux, repli sans JS intact), **R4** décision en masse des hypothèses cochées (endpoint lot, A8 intact), **R6** suppression définitive de session, **R7** accueil « reprise d'abord » + archivées consultables/désarchivables, **R8** Mes documents refondu (tuiles, inventaire groupé par dossier A6, **« marquer obsolète » réversible** — migration 0016 + filtre E2), **R9** projets (édition après création, archivage — migration 0017, **suppression libre** : sessions orphelines, FK SET NULL déjà en place), **R10** « Suivi & réglages » fusionnés (nav 4 entrées). **330 tests verts** (302 → 330), lint vert, CI verte sur le premier push. **Validation stack-live à jouer sur pod : parcours consolidé `docs/plans-test/r0-refonte-parcours-pod.md`** (migrations 0016+0017 incluses, ~30–40 min). Ensuite : inchangé — dépendances externes §7 (snapshot corpus → S3.1/S3.3, gold → S3.4, panel → S3.5), protection de branche `main`, arbitrage modèle actif.

*Voie précédente (07/07/2026, soir)* : **la PR #35 est MERGÉE et son contenu entièrement VALIDÉ SUR POD (« ok fonctionnel » référent)** — 7 stories du 07/07 : **S3.16** formats étendus (pptx/xlsx/eml, convertisseur eml stdlib), **S3.17** suppression de documents / téléchargement de l'original, **S3.18** fix du bug d'association A6 (dossier de destination obligatoire au dépôt), et les **4 correctifs de la session réelle 12** — **S3.19** tableaux géants scindés au chunking (cause racine : le xlsx = 1 chunk de ~940k chars → 312k tokens d'entrée sur 26 appels), **S3.20** garde-fous assemblage E2/affichage, **S3.21** application en lot des levées proposées (A8 assoupli, arbitré), **S3.15** requête RAG contextualisée (14/26 messages sans source) — **302 tests verts** (repo + pod), S3.14→S3.21 cochées au backlog. Avec le lot pré-pilote S3.6→S3.14 (PRs #31→#34, validé pod), **le MVP est prêt pour le pilote côté code**. La suite ne passe plus par le code : dépendances externes §7 (snapshot corpus → S3.1/S3.3, gold → S3.4, panel PO → S3.5), protection de branche `main`, arbitrage modèle actif (la session 12 tournait sur `openweight-large` — medium reste le défaut). Restes mineurs : éval retrieval recall@15 (gated S3.3), compteur télémétrique des stories éditées. Détail : `docs/sprint-3-backlog.md`.

*Voie précédente (07/07/2026, matin)* : **LE LOT PRÉ-PILOTE EST ENTIÈREMENT MERGÉ ET VALIDÉ SUR POD** — PRs #31/#32/#33/#34 (S3.6→S3.14 : rendu markdown, écran réorganisé, htmx, traçabilité A3 persistée avec extrait exact, corpus depuis l'UI + pipeline + suivi, tokens + jauge tpd, modèle changeable depuis Paramètres, édition/renommage/copie, fiche document) — pod validé par le référent (279 verts + écrans OK, photo terminal 07/07).
...
## Session 07/07/2026 (30) — refonte UX/UI : étape 1 (rôle UX/UI + interview PO) puis étape 2 (R1→R10)

**Contexte** : « Nouvelle session. 1ère étape : rôle UX/UI pour réaliser la refonte d'ergonomie avec questionnement du PO ; 2ᵉ étape : développement avec validation des hypothèses avec le PO » — session remote, branche `claude/ux-ui-ergonomie-refonte-fggyen`, **PR #37 (draft)**.

**Étape 1 — UX/UI (aucun code)** :
- Audit des 8 écrans (DSFR superficiel, page-fleuve session ~10 blocs, 3 formulaires empilés, dernière réponse dupliquée, nav sans état actif, bandeau D15 écrasant) — ancré sur les constats des sessions réelles 9/11/12.
- **Interview PO en 3 lots** (AskUserQuestion, méthode maison « lots de 3-4 questions ») → **12 arbitrages UX1–UX12** : chat central dynamique + rail droit EMPILÉ (stories = livrable avec plier/déplier + notation intégrée, hypothèses avec sélection en masse, sources), fil chronologique classique, réponse d'un bloc + indicateur (pas de streaming), archiver+supprimer partout (documents : + obsolète), nav 4 entrées (fusion télémétrie/paramètres), accueil « reprise d'abord », notice D15 une ligne, priorité session+socle, mixte laptop/grand écran, périmètre toute l'app.
- Livrables : **maquette navigable** (artifact + `docs/maquettes/refonte-ux-ui.html`, 3 écrans, données fictives) + **spec `docs/refonte-ux-ui.md`** (journal UX, cible par écran, **registre H1–H15**, découpage R1→R10 en 3 vagues). **Validation PO : « en phase mais conforme DSFR » → UX13** (composants officiels partout, JS DSFR chargé — H15). Commits `c6400d8` + `ec0a440`, poussés, PR #37 draft ouverte, CI verte 4/4.

**Étape 2 — développement (un commit par story, TU + TNR à chaque fois)** :
- **R1** `d2371f8` — socle DSFR : fr-header officiel, fr-nav avec `aria-current` (context processor sur le chemin applicatif, compatible proxy à préfixe), fr-notice D15 une ligne, JS DSFR (H15), fil d'Ariane fiches (3 TU, 305).
- **R2** `feb21ca` — écran session : barre (stepper 0→5 A5, modèle+conso, exports H13, Gérer), chat chronologique complet (H2) à défilement interne ouvert en bas, la réponse du POST rejoint le BAS du fil sans doublon avec le fil persisté S3.9 (H3), composeur unifié (S3.2/règle 5 intactes), rail empilé Stories (notation+édition+copie — **R5 absorbée**) / Hypothèses / Sources (H6) ; 6 TU recalées (objet de la story) + 3 nouvelles (307).
- **R3** `56f685c` — **H7 validée PO** : fragments htmx ciblés (partiels partagés écran/fragment, OOB stepper/conso/3 panneaux, erreur = alerte seule sans OOB, saisie vidée, re-scroll), repli sans JS = pleine page (H14) ; 4 TU + test S3.8 recalé (311).
- **R4** `3d5caa8` — masse hypothèses (H5) : `POST /workflows/{id}/hypotheses/decider-lot` (statut du PO, jamais inventé, seules les cochées en_attente, 409/422), cases rattachées par l'attribut `form`, Tout sélectionner, A8 intact ; 5 TU (316).
- **R6** `895dd8b` — suppression de session : `DELETE /workflows/{id}` (cascades FK 0008/0009/0014/0015, conso dérattachée SET NULL), menu Gérer ; 2 TU (318).
- **R7** `848cad8` — accueil « reprise d'abord » (tableau sessions + actions, création repliée), `GET /workflows?archivees=true`, écran archivées + **désarchivage** ; 3 TU (321).
- **R8** `d65d76b` — Mes documents : **migration 0016** (`est_obsolete`), PATCH document, **filtre `NOT est_obsolete` dans FILTRES_COMMUNS** (BM25+vecteur, réversible sans ré-indexation), tuiles, runs repliés, inventaire groupé par dossier avec projets associés, actions par ligne ; 3 TU + tuples étendus (324).
- **R9** `afea168` — projets : **migration 0017** (`archive`), PATCH/DELETE projet (**H9 tranchée PO : suppression libre** — sessions orphelines via SET NULL 0008 existant, elles continuent sans contexte E8), édition nom/contexte/NFR après création (PUT complet, dossiers A6 préservés), section archivés ; 5 TU (329).
- **R10** — « Suivi & réglages » : page unique 2 sections ancrées, redirections `/telemetrie`→`/suivi#telemetrie` et `/parametres`→`/suivi#parametres`, **nav 4 entrées** (UX9) ; 1 TU (330).

**Validation en session** : lint vert + **330 tests verts** (TNR complet) à chaque story ; CI verte sur les pushes. **Validation stack-live : à jouer sur pod — parcours consolidé `docs/plans-test/r0-refonte-parcours-pod.md`** (pull branche, `alembic upgrade head` pour 0016+0017, `make pod-up`, puis parcours R1→R10 ~30-40 min ; plans détaillés r1→r10 pour les contre-épreuves).

**Mini-récap** :
- ✅ Fait : audit + interview PO (13 arbitrages) + maquette + spec H1–H15 ; **9 stories livrées** (R5 absorbée) — 330 tests ; migrations 0016/0017 ; PR #37 draft à jour
- ⏳ Référent/PO : dérouler `r0-refonte-parcours-pod.md` sur le pod (migrations !), retours d'écart story par story ; revue/merge PR #37
- ⏳ Ensuite : recaler le registre H1–H15 après le pod ; dépendances externes §7 inchangées
...
**Analyse session réelle 12** (capture .htm, retours référent : navigation lourde, aller-retours, hypothèses, conso très élevée « je pense liée à un chunk mal géré d'un tableau excel ») — **piste confirmée, chaîne causale complète** : le xlsx des macro-fonctionnalités = **UN chunk de ~940k caractères** (~235k tokens estimés ; « tableaux jamais coupés » sans borne haute + padding docling à largeur fixe ~75 % d'espaces) → embarqué ENTIER par le **passe-droit du premier chunk** de `assembler_contexte` (`if retenus and…`) → **rerank tué** au passage (« Rerank indisponible ») → moteur qui **signale sans tronquer** (« Budget de contexte dépassé ~239 814 tokens ») → extrait persisté S3.9 qui alourdit chaque page (capture : 1,2 Mo). Conso session : 26 appels, **312 060 tokens entrée** — sur `openweight-large` (surcharge UI, à vérifier). Autres constats : **14 messages sur 26 « aucune source »** (requête = dernier message seul — S3.15 exactement) ; **16 hypothèses en attente** en fin de session. 3 arbitrages rendus (AskUserQuestion) : go S3.19+S3.20, **lot des levées proposées** (A8 assoupli : le PO relit puis applique d'un clic), go S3.15.

**Correctifs livrés (4 stories, un commit chacune, PR #35)** :
- **S3.19 chunking** : `_scinder_tableau` (scission par groupes de lignes, en-tête+séparateur répétés — jamais une ligne coupée ni orpheline) + `_compacter_ligne_tableau` (runs d'espaces) ; ⚠️ corollaire documenté : la reprise sur hash ne re-chunke pas un algorithme changé — propagation = suppression S3.17 + redépôt + ré-indexation (pas de script) ; TU « jamais coupé » réécrite en scission (objet de la story, arbitré) ;
- **S3.20 garde-fous** : fin du passe-droit du premier chunk (tronqué à la borne + marqueur, c'est aussi ce qui part en traçabilité S3.9) ; extraits bornés à 2 000 chars à l'affichage avec mention ; TU « garde toujours le premier chunk » réécrite en troncature (objet du fix) ;
- **S3.21 lot de levées** : `POST /workflows/{id}/hypotheses/appliquer-propositions` (`SET statut = statut_propose` UNIQUEMENT sur en_attente AVEC proposition — jamais de statut inventé, 409 si rien), bouton registre avec confirmation + rappel de relecture, décision individuelle intacte ;
- **S3.15 requête RAG contextualisée** : `construire_requete_recherche` (projet borne 200 + Feature borne 400 + message), dégradée sans projet ; éval recall@15 toujours gated corpus réel (S3.3).
- **302 tests verts** (294 → 302). Plan commun : `docs/plans-test/s3.19-correctifs-session-12.md` (cœur : re-chunk du xlsx via suppression/redépôt, conso d'appel retombée, contre-épreuve A8 SQL du lot).

**Clôture (07/07, fin de journée)** : la **PR #35 est MERGÉE** par le référent (CI verte sur les 8 commits, passage ready → merge dans la foulée), la branche de travail est réalignée sur `main` (`398b6d7`). **Validation stack-live : le référent a déroulé le plan `s3.19-correctifs-session-12.md` sur le pod après pull de `main` — « ok fonctionnel »** (302 tests verts sur pod, re-chunk du xlsx via suppression/redépôt — ce qui valide de facto le champ dossier S3.18 —, conso retombée, lot de levées, sources sur messages courts). **S3.14 → S3.21 cochées au backlog** ; seul reste non coché le volet éval recall@15 de S3.15 (gated corpus réel, S3.3).

**Mini-récap** :
- ✅ Fait : analyses chunking/scoring/contexte ; **7 stories livrées, mergées (PR #35) et validées pod** — S3.16 formats étendus, S3.17 suppression/original, S3.18 fix association A6, S3.19 tableaux scindés, S3.20 garde-fous, S3.21 lot de levées, S3.15 requête contextualisée — 302 tests ; backlog + SESSIONS.md à jour
- ⏳ Référent : dépendances externes §7 (snapshot corpus → S3.1/S3.3, gold → S3.4, panel → S3.5) ; protection de branche `main` ; modèle actif à trancher (large vs medium — session 12 tournait sur large)
- ⏳ Ensuite : éval retrieval recall@15 (gated S3.3) ; compteur télémétrique des stories éditées (mineur)
