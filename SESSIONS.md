...
**Voie active** : sprint 1 — **S1.1, S1.2, S1.4, S1.5 et S1.3 livrées et mergées** (PRs #1–#6). S1.5 : **verdict no-go n°1 GO** (fenêtre gpt-oss-120b 131 072 ≫ budget 20k ; tpm 128k, **tpd 2,46 M = contrainte à surveiller** ; gotchas Albert consignés : max_tokens/raisonnement, **`encoding_format="float"` sur les embeddings SDK**). S1.3 : CI GitHub Actions démontrée verte sur la PR #6 ; **CA2 (protection de branche) = action référent en attente**. **S1.7 et S1.8 mergées (PRs #7, #8 — réserve commune : plans base réelle à jouer ; PDF docling à démontrer sur pod)** ; **S1.9 en cours de livraison (PR #9)** : qualification v0 — métadonnées inférées, doublons par hash, versions regroupées, référence par groupe (migration 0004, `projet_suggere` = suggestion A6), jeu piégé CA4 couvert par TU. **Plans S1.7 → S1.9 enchaînables sur la même base réelle** (mode A lève aussi la réserve compose S1.2). Ensuite : S1.11 (entité Projet), puis S1.6 (Helm), S1.10 (gabarits). **Découverte pod : `ALBERT_API_KEY` est injectée dans l'environnement du pod et prime sur le `.env`** (ALBERT_BASE_URL vient du `.env`). **NB : `make dev` exige un `.env` renseigné** (comportement voulu S1.4). Règle de méthode active : « TU + TNR + plan de test avant toute livraison » (CLAUDE.md). Réserve S1.2 (compose complet) inchangée — étape 7 des plans de test, au plus tard S1.6. Pod de dev : prendre un service `vscode-python` **sans GPU** ; checklist premier login (maxima CPU/RAM, MinIO — action n°7) toujours à consigner. Bascule de la branche par défaut sur `main` : à vérifier dans les settings GitHub.
