...
**Mini-récap** :
- ✅ Fait : analyses chunking/scoring/contexte ; S3.16→S3.18 (validées pod pour 16/17) ; **analyse session 12 + 4 correctifs S3.19/S3.20/S3.21/S3.15** — PR #35, 302 tests ; backlog sprint 3 recalé
- ⏳ Référent : plan s3.19-correctifs-session-12 sur pod (dont **re-dépôt du xlsx** pour le re-chunk et vérif du modèle actif — la session 12 tournait sur large) ; plan s3.18 (chaîne A6) ; revue/merge PR #35
- ⏳ Ensuite : dépendances externes §7 (snapshot corpus, gold, panel PO) ; éval retrieval recall@15 (gated S3.3)
