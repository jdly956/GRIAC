...
**Voie active (06/07/2026, soir)** : **la PR #29 est MERGÉE — la séquence de validation Onyxia est close côté repo.** Première story du backlog d'améliorations issu du réel livrée : **S2.13 — rapprochement décision d'interview ↔ registre A8 (« levée proposée »)** (session 27, commit `8b35dec`, **220 tests verts**) : le moteur reçoit le registre des hypothèses en attente dans son prompt et **propose** la levée quand un message du PO la tranche (`[LEVÉE PROPOSÉE : #id — confirmée|rejetée — justification]`, persistée par la migration 0010) **sans jamais toucher le statut** — la décision individuelle du PO reste le seul chemin de levée (invariant A8 vérifié par TU) ; badge « Levée proposée » sur l'écran session, à côté des boutons. Validation stack-live à jouer via `docs/plans-test/s2.13-rapprochement-a8.md` (pod, clé + base, migration 0010). **En attente référent** : (1) revue/merge de la PR S2.13 ; (2) **arbitrage modèle chat** (`ALBERT_MODEL_CHAT=openweight-medium` à l'essai — verdict E6 v0 : 0,806 vs 0,498 — ou statu quo jusqu'au gold ; le défaut du code reste `openweight-large`) ; (3) protection de branche `main` (CA2 S1.3) ; (4) réserve compose S1.2 (hôte Docker) ; (5) déploiement Helm lab (registre d'images) ; (6) prérequis §7 (stories gold, snapshot corpus réel, panel PO pilotes) ; (7) arbitrage PostgreSQL 16 vs 18.3 CNPG pour la prod. **Backlog d'améliorations restant** : adhérence format d'`openweight-large` (secondaire si bascule medium) ; relance-idempotence du scan (plan s1.7 étape 4, validation pod).
...
## Session 06/07/2026 (27) — S2.13 : rapprochement décision d'interview ↔ registre A8 (« levée proposée »)
...
**Validation en session** : lint vert, **220 tests verts** (TNR complet). Validation stack-live à jouer post-merge (plan s2.13 — exige clé + base, migration 0010).

**Mini-récap** :
- ✅ Fait : analyse doc + plan de travail ; S2.13 livrée (le moteur propose, le PO décide — A8 intact)
- ⏳ Référent : revue/merge PR S2.13 ; arbitrage modèle chat (medium 0,806 vs large 0,498) ; protection de branche ; compose S1.2 ; Helm lab ; prérequis §7 ; arbitrage PG 16 vs 18.3
- ⏳ Backlog : adhérence format de large (secondaire si bascule medium) ; relance-idempotence scan (s1.7 étape 4) ; plan s2.13 à jouer sur pod
