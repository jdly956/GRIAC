...
**Voie active** : sprint 1 — **S1.1, S1.2, S1.4, S1.5 et S1.3 livrées et mergées** (PRs #1–#6). S1.5 : **verdict no-go n°1 GO** (fenêtre gpt-oss-120b 131 072 ≫ budget 20k ; tpm 128k, **tpd 2,46 M = contrainte à surveiller** ; gotchas Albert consignés : max_tokens/raisonnement, **`encoding_format="float"` sur les embeddings SDK**). S1.3 : CI GitHub Actions démontrée verte sur la PR #6 ; **CA2 (protection de branche) = action référent en attente**. **S1.7 mergée (PR #7, réserve : plan base réelle à jouer)** ; **S1.8 en cours de livraison (PR à venir)** : parsing docling → dérivés `derived/md/<sha256>.md`, statuts en base (migration 0003), `ocr_requis` pour les PDF sans texte, reprise sur hash D9, rapport d'échecs — conversion docx réelle démontrée en session, **PDF à démontrer sur pod** (modèles docling téléchargés au 1er run, bloqués par le proxy de session). **Plans S1.7 + S1.8 enchaînables sur base réelle** (`docs/plans-test/s1.7-ingest-scan.md` puis `s1.8-ingest-parse.md` — mode A lève aussi la réserve compose S1.2). Ensuite : S1.9 (qualification v0). **Découverte pod : `ALBERT_API_KEY` est injectée dans l'environnement du pod et prime sur le `.env`** (ALBERT_BASE_URL vient du `.env`). **NB : `make dev` exige un `.env` renseigné** (comportement voulu S1.4). Règle de méthode active : « TU + TNR + plan de test avant toute livraison » (CLAUDE.md). Réserve S1.2 (compose complet) inchangée — étape 7 des plans de test, au plus tard S1.6. Pod de dev : prendre un service `vscode-python` **sans GPU** ; checklist premier login (maxima CPU/RAM, MinIO — action n°7) toujours à consigner. Bascule de la branche par défaut sur `main` : à vérifier dans les settings GitHub.
