...
**Mini-récap** :
- ✅ Fait : analyse doc + plan de travail ; S2.13 livrée (`8b35dec`) **et validée stack-live (session 11)** ; S2.14 livrée (`f35b538`, corroborée sessions 9/11) ; correctifs session 9 (`4ce5bd4`) + session 11 ; TNR 225 verts ; PR #30 draft
- ⏳ Référent : revue/merge PR #30 ; clics Confirmer/Rejeter + recoupe SQL (facultatif, plan s2.13 §4-5) ; protection de branche ; compose S1.2 ; Helm lab ; prérequis §7 ; arbitrage PG 16 vs 18.3
- ⏳ Backlog : anti-invention (variantes de marqueur + reformulations du registre + valeurs inventées) ; sémantique du « Oui » (story vs étape — arbitrage produit) ; adhérence format de large (si l'essai medium déçoit) ; relance-idempotence scan (s1.7 étape 4)
